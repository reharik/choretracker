...
export const createChoreRoutes = ({
  ...
  mountRoutes: (router: Router) => {
    ...
    router.get(
      "/chores/bonus-settings",
      requireAuth(choreController.getBonusSettings),
    );
    router.patch(
      "/chores/bonus-settings",
      requireAuth(choreController.updateBonusSettings),
    );

    // Snapshots (payday)
    router.post(
      "/chores/snapshot",
      requireAuth(choreController.createSnapshot),
    );
    ...
    router.patch(
      "/chores/:id",
      requireAuth(typedHandler(choreController.updateChore)),
    );
    router.delete(
      "/chores/:id",
      requireAuth(typedHandler(choreController.deleteChore)),
    );
    ...
    router.delete(
      "/chores/extras/:id",
      requireAuth(typedHandler(choreController.deleteExtra)),
    );

    // Toggle check on/off
    router.post(
      "/chores/checks/toggle",
      requireAuth(choreController.toggleCheck),
    );
  },
});
