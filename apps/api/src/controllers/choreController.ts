...
export const createChoreController = ({
  ...
  toggleCheck: async (ctx: Context): Promise<Context> => {
    ...
    if (!weekKey || dayIndex == null || (!choreId && !choreExtraId)) {
      ...
      ctx.body = {
        error:
          "weekKey, dayIndex, and either choreId or choreExtraId are required",
      };
      return ctx;
    }
  ...
  updateBonusSettings: async (ctx: Context): Promise<Context> => {
    const userId = ctx.user!.id;
    const { overCompletionBonusPercent, allChoresCompleteBonusPercent } =
      ctx.request.body;
    ...
    if (
      ...
      ctx.status = 400;
      ctx.body = {
        error: "overCompletionBonusPercent must be between 0 and 100",
      };
      return ctx;
    }
    ...
    if (
      ...
      ctx.status = 400;
      ctx.body = {
        error: "allChoresCompleteBonusPercent must be between 0 and 100",
      };
      return ctx;
    }
