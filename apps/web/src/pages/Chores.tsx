...
function getCurrentWeekKey(): string {
  ...
  const daysDiff = Math.floor(
    (now.getTime() - week1Monday.getTime()) / 86400000,
  );
...
function addWeeks(weekKey: string, weeks: number): string {
  ...
  const daysDiff = Math.floor(
    (targetMonday.getTime() - targetWeek1Monday.getTime()) / 86400000,
  );
...
export const Chores = () => {
    ...
    if (result.success) {
      ...
      setBonusSettings({
        overCompletionBonusPercent:
          result.data.bonusSettings.overCompletionBonusPercent,
        allChoresCompleteBonusPercent:
          result.data.bonusSettings.allChoresCompleteBonusPercent,
      });
  ...
  const handleToggle = async (
    ...
    await choreService.toggleCheck({
      choreId,
      choreExtraId,
      weekKey,
      dayIndex,
  ...
  const handleSaveChore = async (id: string) => {
    if (
      !editChoreData.name ||
      !editChoreData.baseValue ||
      !editChoreData.timesPerWeek
    )
      return;
  ...
  const isChecked = (
    ...
    return summary.checks.some(
      (c: ChoreCheck) =>
        c.dayIndex === dayIndex &&
        ((choreId && c.choreId === choreId) ||
          (choreExtraId && c.choreExtraId === choreExtraId)),
    );
  };
      ...
      summary.extras.length > 0
        ? summary.extras
            .map((ex: ExtraWithStats) => {
              const boxes = DAYS.map((_, di) => {
                const checked = showCurrent
                  ? isChecked(undefined, ex.id, di)
                  : false;
                return `<td style="text-align:center;padding:5px;">
  ...
  if (loading) {
    return (
      <PageWrapper>
        <p style={{ color: "#a8b3c7", textAlign: "center", padding: "40px 0" }}>
          Loading...
        </p>
      </PageWrapper>
    );
  ...
  return (
    <PageWrapper>
      ...
      {editMode && (
        <Card
          style={{ marginBottom: 16, borderColor: "rgba(250,204,21,0.25)" }}
        >
          <VStack gap={2}>
            <h3 style={{ fontSize: "0.9rem", color: "#facc15", margin: 0 }}>
              Edit Base Chores
            </h3>
            {summary.chores.map((c: ChoreWithStats) => (
              <div key={c.id}>
                  ...
                  >
                    ...
                    <HStack gap={1}>
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => handleStartEditChore(c)}
                      >
                        Edit
                      </Button>
                      <Button
            ...
            <VStack gap={2}>
              <div>
                ...
                <HStack gap={1}>
                  ...
                  <span style={{ fontSize: "0.8rem", color: "#a8b3c7" }}>
                    % bonus on top of 100% for each completion beyond required
                    (e.g., 25% = 125% total)
                  </span>
                </HStack>
              </div>
              ...
              <Button
                ...
                onClick={async () => {
                  const result =
                    await choreService.updateBonusSettings(bonusSettings);
                  if (result.success) {
                    await loadSummary();
                  }
                }}
          ...
          <GridRow key={chore.id} $alt={i % 2 === 1}>
            <div>
              ...
              <ChoreDetail>
                ...
                {chore.completions > 0 && (
                  <CompletionBadge
                    $met={chore.completions >= chore.timesPerWeek}
                  >
            ...
            <Earned $hasValue={chore.earned > 0}>
              ${chore.earned.toFixed(2)}
            </Earned>
          </GridRow>
        ))}

        {summary.extras.length > 0 && (
          <SectionLabel>⭐ Extra Chores</SectionLabel>
        )}
        {summary.extras.map((ex: ExtraWithStats) => (
      ...
      <LegendCard style={{ marginTop: 14 }}>
        ...
        <br />
        ✓ Each chore earns its $ value when done the required # of times per
        week.
        ...
        <br />⭐ <span style={{ color: "#f97316" }}>Extra chores</span> are
        elective one-offs — add them in Edit mode.
      </LegendCard>
    </PageWrapper>
  );
};
