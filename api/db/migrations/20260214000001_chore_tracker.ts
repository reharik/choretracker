...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('chore_checks', (t) => {
    t.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    t.uuid('userId').notNullable().references('id').inTable('users').onDelete('CASCADE');
    t.uuid('choreId').nullable().references('id').inTable('chores').onDelete('CASCADE');
    t.uuid('choreExtraId').nullable().references('id').inTable('chore_extras').onDelete('CASCADE');
    t.string('weekKey', 10).notNullable();
    t.integer('dayIndex').notNullable(); // 0=Mon .. 6=Sun
    t.timestamp('checkedAt', { useTz: true }).notNullable().defaultTo(knex.fn.now());
